...
import textClassiferModelMulti as model
import time
import os
...
def train():
    ...
    for epoch in range(gConfig['epochs']):
        ...
        for (batch,(inp1,inp2,inp3,target)) in enumerate(dataset_train.batch(gConfig['batch_size'])):
            ...
            print('train:Epoch {} Batch {} Loss {:.4f} AUC {:.4f} prestep {:.4f}'.format(epoch+1,batch,loss[0],loss[1],(time.time()-start)))
        ...
        for (batch,(inp1,inp2,inp3,target)) in enumerate(dataset_test.batch(gConfig['batch_size'])):
            ...
            print('validate:Epoch {} Batch {} Loss {:.4f} AUC {:.4f} prestep {:.4f}'.format(epoch+1,batch,loss[0],loss[1],(time.time()-start)))
...
def prediction():
    model = create_model(best_ckpt=1)
    ...
    for file in files:
        ...
        print(file,' ',cal_ks(output[:,0],output[:,1])[0])
        np.savetxt(gConfig['data_path'] + file +'_prediction_all',output,delimiter=',')
