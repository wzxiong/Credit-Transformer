...
class TransformerMulti(tf.keras.Model):
    def __init__(self, num_feat1, num_feat2, num_layers, d_model, dff, num_heads, input_vocab_size, rate=0.1):
        ...
        self.enc_layers = [EncoderLayer(24, dff, num_heads, rate)
                           for _ in range(num_layers)]
    ...
    def call(self, inp1, inp2, inp3, training, enc_padding_mask):
        ...
        feat2_index = tf.cast(np.array([range(gConfig['num_feat2'])]*int(tf.shape(inp1)[0])), tf.float32)
        ...
        inp2_emb = tf.reshape(inp2, (-1,39,24))
        ...
        inp2_output = tf.reshape(inp2_emb,(-1,gConfig['num_feat2']))
...
#transformerMulti_para = multi_gpu_model(transformerMulti, gpus=4)
